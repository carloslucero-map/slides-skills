...
claude.ai):
  ...
  1. Read the design system with the Artifact tool into one folder: every text
     file its file listing shows (one `paths` call: the README, the index, the
     tokens, the stylesheet, the guidelines, each asset group's README.md and
     each card's README.md and preview.html; MANIFEST below names the ones this
     script cannot find on its own), and every asset in design-system.json's
     asset groups (one `path` call per blob id; the tool saves each as
     <blob><ext> beside project/).
  2. python3 authoring/refresh_design_system.py --from <that folder> \\
         --artifact <design system url> --version <version id> --write
  3. Prove nothing moved that should not have: check_shared_blocks.py,
...
What it writes, under wpp-es-html-deck/:
  ...
  design-system/README.md, guidelines/            verbatim: the brand in brief and the
                                                  eleven guidelines, what the model reads
  design-system/cards/<group>.md                  every card's README, verbatim, one file
                                                  per group (elements, fixed slides, and
                                                  each layout family), in the README's order
  design-system/asset-notes.md                    each asset group's README, verbatim
...
# Everything read by path. The rest is found from the folder itself: the
# guidelines (project/guidelines/*.md), every card (project/components/<Name>/
# holding README.md and preview.html, grouped by its preview's @dsCard line),
# and the layout cards among them (a README whose Origin line names a skill
# file). The asset groups' READMEs are named by design-system.json's groups.
MANIFEST = [
...
# Card groups -> the cache file that holds them; "Layouts · <Family>" groups go
# to cards/layouts-<family>.md. A group not named here stops the refresh.
CARD_FILES = {"Elements": "cards/elements.md", "Fixed slides": "cards/fixed-slides.md"}
CARD_BANNER = ("<!-- Generated by authoring/refresh_design_system.py: the design system's "
               "{what}, verbatim{order}. Do not edit; change the design system, then refresh. -->\n")
...
# ── the brand rules, for the model that builds the deck ──────────────────────

def card_files(src):
    """{cache path: (text, from)}: every card's README, one file per group, each in
    the order the design system's README first names the card."""
    readme = read(src, "project/README.md")
    groups = {}
    for p in sorted(glob.glob(os.path.join(src, "project", "components", "*", "README.md"))):
        name = os.path.basename(os.path.dirname(p))
        preview = os.path.join(os.path.dirname(p), "preview.html")
        m = (re.search(r'@dsCard group="([^"]+)"', open(preview, encoding="utf-8").read())
             if os.path.isfile(preview) else None)
        if not m:
            raise Fail(f"card {name} has no preview.html naming its group (read it too)")
        groups.setdefault(m.group(1), []).append(name)
    out = {}
    for group, names in sorted(groups.items()):
        if group in CARD_FILES:
            rel = CARD_FILES[group]
        elif group.startswith("Layouts · "):
            rel = "cards/layouts-" + group.split(" · ", 1)[1].lower().replace(" ", "-") + ".md"
        else:
            raise Fail(f"cards in a group this script does not know: {group!r} ({', '.join(names)})")
        order = sorted(names, key=lambda n: (readme.find(f"`{n}`") if f"`{n}`" in readme
                                             else len(readme), n))
        body = "\n".join(read(src, f"project/components/{n}/README.md").rstrip("\n") + "\n"
                         for n in order)
        out[rel] = (CARD_BANNER.format(what=f"{group} cards",
                                       order=", in the order its README names them") + "\n" + body,
                    f"the {group} cards' README.md ({len(order)})")
    return out


def asset_notes(src, ds):
    """Each asset group's README, in the index's group order."""
    body = "\n".join(read(src, f"project/assets/{g}/README.md").rstrip("\n") + "\n"
                     for g in ds["groups"])
    return CARD_BANNER.format(what="notes on each asset group", order="") + "\n" + body
...
def plan(src, artifact, version):
    ...
    files[f"{CACHE}/README.md"] = (read(src, "project/README.md", binary=True), "project/README.md")
    guides = sorted(glob.glob(os.path.join(src, "project", "guidelines", "*.md")))
    if not guides:
        raise Fail("missing from the read: project/guidelines/*.md")
    for g in guides:
        rel = "project/guidelines/" + os.path.basename(g)
        files[f"{CACHE}/guidelines/{os.path.basename(g)}"] = (read(src, rel, binary=True), rel)
    for rel, (text, where) in card_files(src).items():
        files[f"{CACHE}/{rel}"] = (text.encode("utf-8"), where)
    files[f"{CACHE}/asset-notes.md"] = (asset_notes(src, ds).encode("utf-8"),
                                        "each asset group's README.md")
