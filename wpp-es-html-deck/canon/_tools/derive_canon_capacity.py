...
learn a second format.
  ...
  python3 canon/_tools/derive_canon_capacity.py --demo <a-deck>.html --write
  python3 canon/_tools/derive_canon_capacity.py --demo <a-deck>.html --check
...
def build(demo):
    ...
    out = []
    for d in sorted(os.listdir(CANON)):
        p = os.path.join(CANON, d, "template.html")
    ...
    if args.check:
        ...
        if len(have) != len(canon):
            print(f"CANON CAPACITY STALE — {len(canon)} templates on disk, "
                  ...
                  f"canon/_tools/derive_canon_capacity.py --demo <deck>.html --write",
                  file=sys.stderr)
            sys.exit(1)
